...
export const BeyondThePodium: React.FC = () => {
  ...
  return <section ref={containerRef} className="relative w-full min-h-screen bg-[#0F0F0F] text-[#F5F5F0] overflow-hidden" aria-labelledby="beyond-the-podium-heading">
      <style>{`
        ...
        .tsf-dialogue-card {
          transition: transform 380ms ease-in-out, box-shadow 380ms ease-in-out, border-color 380ms ease-in-out;
        }
        .tsf-dialogue-card:hover {
      ...
      <div className="container mx-auto px-6 py-20 sm:py-24 md:px-12 md:py-28 lg:py-[140px] flex flex-col justify-center min-h-screen max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3 md:gap-6">
          {DIALOGUE_SERIES_DATA.map((series, index) => <motion.article key={series.id} custom={index} initial="hidden" animate={isInView ? "visible" : "hidden"} variants={itemVariants} className={cn("tsf-dialogue-card group relative flex flex-col overflow-hidden rounded-[22px]", "border-b border-[#333333] pb-12 lg:border-b-0 lg:pb-0")}>
              <figure className="m-0">
                <div className={cn("tsf-cinematic-preview relative flex aspect-video w-full items-center justify-center overflow-hidden rounded-[18px] border border-[#333333] transition-colors duration-[400ms] ease-in-out group-hover:border-[#e30e04]/70", series.previewClassName)} aria-label={`${series.speakerName} cinematic video preview`} role="img">
                  ...
                  <div aria-hidden="true" className="absolute inset-0 z-[6] bg-[linear-gradient(180deg,transparent_0%,rgba(0,0,0,0.65)_100%)] opacity-0 transition-opacity duration-[400ms] ease-in-out group-hover:opacity-100" />
                  ...
                  <div className="absolute bottom-5 left-5 right-5 z-[7] transition-transform duration-[400ms] ease-in-out group-hover:-translate-y-2">
                    ...
                    <div className="mt-4 inline-flex translate-y-3 items-center gap-2 rounded-full bg-[#e30e04] px-4 py-2 text-[10px] font-bold uppercase tracking-[0.14em] text-white opacity-0 shadow-[0_12px_32px_rgba(227,14,4,0.26)] transition-all duration-[400ms] ease-in-out group-hover:translate-y-0 group-hover:opacity-100">
                      <span>Book Now</span>
                      <ArrowRight aria-hidden="true" className="h-3.5 w-3.5" />
                    </div>
                  </div>
                </div>
                <figcaption className="mt-7 transition-transform duration-[400ms] ease-in-out group-hover:-translate-y-1">
