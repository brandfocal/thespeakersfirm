...
import { ArrowRight, BookOpen, BriefcaseBusiness, CalendarDays, CheckCircle, ChevronDown, ChevronLeft, ChevronRight, Clock, Globe, Menu, Play, Quote, Send, X } from "lucide-react";
...
const BOOKS = [{
  ...
  accent: "#e30e04",
  coverClassName: "from-[#0F0F0F] via-[#393939] to-[#212121]",
  ...
  author: "Professor Bonang Mohale",
  accent: "#e30e04",
  coverClassName: "from-[#212121] via-[#393939] to-[#0F0F0F]",
...
const MINI_CALENDAR_DAYS: CalendarDay[] = [{
  id: "inline-may-27",
  day: 27,
  isAvailable: false,
...
const AbstractPortrait = () => {
  return <figure className="group relative isolate flex min-h-[clamp(280px,90vw,500px)] w-full max-w-[500px] flex-col overflow-hidden rounded-[22px] border transition-colors duration-[400ms] ease-out hover:border-[#e30e04]/70 sm:min-h-[clamp(340px,70vw,500px)] lg:h-[calc(100vh-128px)] lg:max-h-[560px]" style={{
    backgroundColor: COLORS.deepBlack,
    borderColor: COLORS.darkGray,
    boxShadow: "0 24px 70px rgba(33, 33, 33, 0.18)"
  }} aria-label="Portrait for Professor Bonang Mohale">
      <img className="absolute inset-0 h-full w-full object-cover object-top" src="https://images.unsplash.com/photo-1475721027785-f74eccf877e2?auto=format&fit=crop&w=1200&q=85" alt="Distinguished professional male speaker at a podium in dramatic stage lighting" />
    ...
    ease: [0.22, 1, 0.36, 1]
      ...
      <div className="flex min-h-[500px] flex-col md:flex-row">
        ...
        <div className="flex-1 p-4 sm:p-6 md:p-8" aria-label="June 2024 booking calendar">
          {bookingStep === "slot" ? <motion.div key="booking-slot-step" initial={{
          opacity: 0,
          y: 14
        }} animate={{
          opacity: 1,
          ...
          ease: "easeOut"
              ...
              <div className="grid grid-cols-7 gap-1 sm:gap-2" role="grid" aria-label="June 2024 available dates">
                {WEEKDAYS.map(weekday => <div key={weekday.id} className="pb-2 text-center text-[9px] font-bold text-[#555] sm:pb-4 sm:text-[10px]" role="columnheader">
                    <span>{weekday.label}</span>
                  </div>)}
                {CALENDAR_DAYS.map(day => {
              const isSelected = selectedDateId === day.id;
              return <button key={day.id} type="button" disabled={!day.isAvailable || !day.isCurrentMonth} aria-pressed={isSelected} aria-label={`${day.isCurrentMonth ? "June" : "May"} ${day.day}${day.isAvailable && day.isCurrentMonth ? ", available" : ""}${isSelected ? ", selected" : ""}`} onClick={() => handleDateSelect(day)} className={`group relative flex aspect-square min-h-9 items-center justify-center rounded-full text-xs font-medium transition-all focus:outline-none focus:ring-2 focus:ring-[#e30e04] focus:ring-offset-2 focus:ring-offset-[#1A1A1A] sm:text-sm ${day.isCurrentMonth ? day.isAvailable ? "text-white" : "text-[#555]" : "text-[#333]"} ${day.isAvailable && day.isCurrentMonth ? "cursor-pointer hover:bg-[#e30e04] hover:text-white" : "cursor-default"} ${isSelected ? "border border-[#e30e04] bg-[#e30e04] text-white shadow-[0_0_0_4px_rgba(227,14,4,0.16)]" : ""}`}>
                    <span>{day.day}</span>
                    {day.isAvailable && day.isCurrentMonth ? <span className={`absolute bottom-1.5 left-1/2 h-1 w-1 -translate-x-1/2 rounded-full ${isSelected ? "bg-white" : "bg-[#e30e04] group-hover:bg-white"}`} aria-hidden="true" /> : null}
                  </button>;
            })}
              </div>

              {selectedDate ? <motion.section initial={{
            opacity: 0,
            y: 16
          }} animate={{
            opacity: 1,
            ...
            duration: 0.32,
            ease: "easeOut"
          }} className="mt-8 border-t border-[#333] pt-7" aria-labelledby="sast-time-picker-heading">
                ...
                return <button key={slot.id} type="button" aria-pressed={isSelected} onClick={() => setSelectedTimeId(slot.id)} className={`rounded-full border px-4 py-3 text-sm font-bold transition-all focus:outline-none focus:ring-2 focus:ring-[#e30e04] focus:ring-offset-2 focus:ring-offset-[#1A1A1A] ${isSelected ? "border-[#e30e04] bg-[#e30e04] text-white" : "border-[#333] bg-[#212121] text-[#AFB0B0] hover:border-[#e30e04] hover:text-white"}`}>
              ...
              <div className="mt-8 flex flex-col gap-3 border-t border-[#333] pt-6 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button type="button" disabled={!canContinue} onClick={() => setBookingStep("details")} className={`inline-flex h-[48px] items-center justify-center rounded-full px-6 text-[11px] font-bold uppercase tracking-[0.1em] transition-all sm:h-[52px] sm:px-7 sm:text-[12px] ${canContinue ? "bg-[#e30e04] text-white hover:opacity-90" : "cursor-not-allowed bg-[#333] text-[#777]"}`} aria-label="Continue to your details">
                  <span>Continue</span>
                </button>
              </div>
            </motion.div> : null}

          {bookingStep === "details" ? <motion.form key="booking-details-step" initial={{
          opacity: 0,
          y: 14
        }} animate={{
          opacity: 1,
          ...
          ease: "easeOut"
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  ...
                  <input className="w-full rounded-xl border border-[#444] bg-[#393939] px-5 py-4 text-base text-white placeholder-[#666] transition-colors focus:border-[#e30e04] focus:outline-none" name="bookingFullName" type="text" autoComplete="name" value={bookingDetails.fullName} onChange={event => handleDetailsChange("fullName", event.target.value)} required />
                </label>
                <label className="block">
                  <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-[#AFB0B0]">Email Address</span>
                  <input className="w-full rounded-xl border border-[#444] bg-[#393939] px-5 py-4 text-base text-white placeholder-[#666] transition-colors focus:border-[#e30e04] focus:outline-none" name="bookingEmail" type="email" autoComplete="email" value={bookingDetails.email} onChange={event => handleDetailsChange("email", event.target.value)} required />
                </label>
                <label className="block sm:col-span-2">
                  <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-[#AFB0B0]">Phone Number</span>
                  <input className="w-full rounded-xl border border-[#444] bg-[#393939] px-5 py-4 text-base text-white placeholder-[#666] transition-colors focus:border-[#e30e04] focus:outline-none" name="bookingPhone" type="tel" autoComplete="tel" value={bookingDetails.phone} onChange={event => handleDetailsChange("phone", event.target.value)} required />
                </label>
                <label className="block sm:col-span-2">
                  <span className="mb-2 block text-xs font-bold uppercase tracking-widest text-[#AFB0B0]">What would you like to discuss?</span>
                  <textarea className="min-h-[132px] w-full resize-none rounded-xl border border-[#444] bg-[#393939] px-5 py-4 text-base text-white placeholder-[#666] transition-colors focus:border-[#e30e04] focus:outline-none" name="bookingDiscussion" value={bookingDetails.discussion} onChange={event => handleDetailsChange("discussion", event.target.value)} />
                </label>
              </div>
              <div className="mt-8 flex flex-col-reverse gap-3 border-t border-[#333] pt-6 sm:flex-row sm:items-center sm:justify-between">
                <button type="button" onClick={() => setBookingStep("slot")} className="inline-flex h-[48px] items-center justify-center rounded-full border border-[#AFB0B0] px-6 text-[11px] font-bold uppercase tracking-[0.1em] text-[#AFB0B0] transition-colors hover:border-white hover:text-white sm:h-[52px] sm:px-7 sm:text-[12px]" aria-label="Back to date and time selection">
                  <span>Back</span>
                </button>
                <button type="submit" className="inline-flex h-[48px] items-center justify-center rounded-full bg-[#e30e04] px-6 text-[11px] font-bold uppercase tracking-[0.1em] text-white transition-opacity hover:opacity-90 sm:h-[52px] sm:px-7 sm:text-[12px]" aria-label="Confirm booking">
          ...
          }} className="mt-8 inline-flex h-[52px] items-center justify-center rounded-full border border-[#AFB0B0] px-7 text-[12px] font-bold uppercase tracking-[0.1em] text-[#AFB0B0] transition-colors hover:border-white hover:text-white" aria-label="Make another booking">
...
const BookingSection = () => {
  return <section id="booking-calendar" className="relative flex min-h-screen w-full flex-col items-center overflow-hidden bg-[#212121]" aria-labelledby="booking-section-heading">
...
const SpeakerEnquirySection = () => {
  return <motion.section id="enquiry-form" initial={{
    opacity: 0,
    y: 30
  }} whileInView={{
    opacity: 1,
    ...
    duration: 0.7,
    ease: "easeOut"
  }} className="relative overflow-hidden bg-[#212121]" aria-labelledby="enquiry-form-heading">
      ...
      <div className="relative z-10 mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 py-16 sm:px-6 md:px-8 md:py-24 lg:grid-cols-2">
        ...
        <form className="rounded-[28px] bg-[#2a2a2a] p-5 sm:p-8 md:rounded-[32px] md:p-10" aria-label="Speaker enquiry form">
          <div className="space-y-5">
            ...
            delay: 0.36,
            ease: "easeOut"
          }} className="mt-2 flex h-[56px] w-full items-center justify-center gap-3 rounded-full bg-[#e30e04] text-[12px] font-bold uppercase tracking-[0.1em] text-white transition-opacity hover:opacity-90" type="submit" aria-label="Submit speaker enquiry">
  ...
  return <div className="w-full overflow-x-hidden font-['Kontora',sans-serif]">
    <section className="relative flex min-h-screen w-full flex-col items-center overflow-hidden px-4 py-0 font-['Kontora',sans-serif] sm:px-6 lg:px-16 xl:px-0" style={{
      ...
      <div className="relative z-20 mx-auto w-full max-w-[1312px] pt-4 sm:pt-6">
        <nav className="flex items-center justify-between border-b py-3" style={{
          borderColor: COLORS.borderGray
        }} aria-label="Primary navigation">
          <button className="text-left text-[11px] font-bold uppercase leading-tight tracking-[0.16em] sm:text-[12px]" style={{
            color: COLORS.black
          ...
          <div className="hidden items-center gap-6 md:flex">
            {NAV_ITEMS.map(item => <button key={item.id} className="text-[11px] font-bold uppercase tracking-[0.14em] transition-colors hover:text-[#e30e04]" style={{
              color: COLORS.black
            }} type="button" onClick={() => scrollToSection(item.targetId)}>
                <span>{item.label}</span>
              </button>)}
          </div>
          <button className="inline-flex h-11 w-11 items-center justify-center rounded-full border md:hidden" style={{
            borderColor: COLORS.borderGray,
            color: COLORS.black
          }} type="button" aria-label={isMobileNavOpen ? "Close navigation menu" : "Open navigation menu"} aria-expanded={isMobileNavOpen} onClick={() => setIsMobileNavOpen(!isMobileNavOpen)}>
            {isMobileNavOpen ? <X className="h-5 w-5" aria-hidden="true" /> : <Menu className="h-5 w-5" aria-hidden="true" />}
          </button>
        </nav>
        {isMobileNavOpen ? <div className="border-b py-3 md:hidden" style={{
          borderColor: COLORS.borderGray
        }}>
            <div className="grid gap-2" aria-label="Mobile navigation links">
              {NAV_ITEMS.map(item => <button key={item.id} className="flex min-h-12 items-center justify-between rounded-full px-4 text-left text-[11px] font-bold uppercase tracking-[0.14em] transition-colors hover:bg-[#212121] hover:text-white" style={{
              color: COLORS.black
            }} type="button" onClick={() => scrollToSection(item.targetId)}>
                  <span>{item.label}</span>
                  <ArrowRight className="h-4 w-4" aria-hidden="true" />
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1312px]">
        <div className="grid items-start gap-8 py-8 md:py-9 lg:min-h-[calc(100vh-76px)] lg:grid-cols-12 lg:gap-10 lg:py-8 xl:py-9">
          <motion.div initial={{
            opacity: 0,
            y: 20
          }} whileInView={{
            ...
            duration: 0.6,
            ease: [0.22, 1, 0.36, 1]
          }} className="flex flex-col justify-center space-y-5 pt-0 lg:col-span-7 lg:min-h-[calc(100vh-148px)] lg:space-y-5">
            <div>
              <motion.div initial={{
                opacity: 0,
                scale: 0.95
                ...
                delay: 0.1,
                duration: 0.4
              }} className={SECTION_TAG_CLASS} style={SECTION_TAG_STYLE}>
            ...
            <div className="space-y-4">
              ...
              <motion.div initial={{
                ...
                delay: 0.18,
                ease: [0.16, 1, 0.3, 1]
              }} className="h-[2px] w-24 origin-left md:w-32" style={{
                backgroundColor: COLORS.red
              }} aria-hidden="true" />
            </div>
            ...
            <div className="max-w-[65ch] space-y-4">
              <ul className="space-y-0 border-y" style={{
                ...
                {CURRENT_ROLES.map(role => <li key={role.id} className="flex items-start gap-4 border-b py-2.5 last:border-b-0 md:items-center lg:py-2" style={{
                  borderColor: SOFT_RULE_COLOR
                }}>
          ...
          <motion.div initial={{
            ...
            delay: 0.2,
            ease: [0.22, 1, 0.36, 1]
          }} className="flex w-full justify-center lg:col-span-5 lg:min-h-[calc(100vh-72px)] lg:items-center lg:justify-end">
            <AbstractPortrait />
          </motion.div>
        </div>
        ...
        <motion.div initial={{
          ...
          duration: 0.65,
          ease: [0.22, 1, 0.36, 1]
        }} className="grid gap-10 py-12 md:py-20 lg:grid-cols-12 lg:gap-16" id="the-brief">
          ...
          <div className="lg:col-span-8">
            <div className="max-w-[65ch] space-y-5 border-l-2 pl-5 md:pl-7" style={{
              ...
              <motion.div initial={{
                ...
                <p className={LEAD_TEXT_CLASS} style={{
                  color: COLORS.black
                }}>
                  <span>{BIO_HOOK}</span>
                </p>
                {isBioExpanded ? <p id="full-biography" className={BODY_TEXT_CLASS} style={{
                  color: COLORS.gray
                }}>
                    <span>{FULL_BIO}</span>
                  </p> : null}
              ...
              <motion.div initial={{
                ...
                <button className={`inline-flex items-center rounded-full border border-[#212121] px-5 py-3 text-[#212121] transition-colors duration-300 hover:bg-[#212121] hover:text-white ${CTA_TEXT_CLASS}`} type="button" aria-expanded={isBioExpanded} aria-controls="full-biography" onClick={() => setIsBioExpanded(!isBioExpanded)}>
        ...
        <motion.section initial={{
          ...
          duration: 0.65,
          ease: [0.22, 1, 0.36, 1]
        }} className="relative left-1/2 w-screen -translate-x-1/2 overflow-hidden bg-[#212121] px-4 py-16 text-white sm:px-6 md:px-16 md:py-32" aria-labelledby="boardroom-section-heading" id="boardroom-interventions">
            ...
            <div className="flex flex-grow justify-center xl:justify-end">
              <div className="flex w-full flex-col gap-0 overflow-hidden rounded-[24px] md:rounded-[32px]">
                {STRATEGIC_THEMES.map((theme, themeIndex) => {
                  ...
                  return <motion.article key={theme.id} className={`w-full overflow-hidden border-b border-[#393939] last:border-b-0 ${isActive ? "bg-[#2a2a2a]" : "bg-[#212121]"}`} aria-label={`${theme.number} ${theme.title}`}>
                      <button type="button" className="flex w-full cursor-pointer items-start justify-between gap-4 px-5 py-5 text-left transition-colors hover:bg-[#2a2a2a] sm:items-center sm:gap-6 sm:px-8 sm:py-6" aria-expanded={isActive} aria-controls={`${theme.id}-panel`} onClick={() => setActiveStrategicThemeIndex(isActive ? null : themeIndex)}>
        ...
        <motion.section initial={{
          ...
          <div className="bg-[inherit] py-2 lg:sticky lg:top-0 lg:z-10 lg:col-span-4 lg:px-8 lg:py-6">
            <h2 className={SECTION_HEADING_CLASS}>
                ...
                {BOOKS.map(book => <div key={book.id} className="grid gap-5 sm:grid-cols-[132px_1fr] md:grid-cols-1 lg:grid-cols-[150px_1fr]">
                    <figure className="w-full max-w-[132px] lg:max-w-[150px]" aria-label={`${book.coverTitle} book cover`}>
                      ...
                      borderColor: "rgba(248, 247, 245, 0.22)"
                        ...
                        <div className="absolute left-0 top-0 h-full w-[10px] bg-[#0F0F0F]/24" aria-hidden="true" />
                        <div className="absolute inset-x-0 top-0 h-1/3 bg-gradient-to-b from-[#F8F7F5]/18 to-transparent" aria-hidden="true" />
                        <div className="relative flex h-full flex-col justify-between pl-2">
                          <p className="text-[10px] font-bold uppercase leading-[1.25] tracking-[0.22em] text-[#F8F7F5]/78 md:text-[11px]"><span>Leadership / Governance</span></p>
                          <div className="space-y-3">
                            <h4 className={`${KONTORA_FONT_CLASS} text-[clamp(1.375rem,6vw,1.75rem)] font-bold leading-tight tracking-[-0.04em] text-[#F8F7F5]`}><span>{book.coverTitle}</span></h4>
                            <div className="h-[2px] w-12" style={{
                            backgroundColor: book.accent
                          }} aria-hidden="true" />
                          </div>
                          <p className="text-[10px] font-bold uppercase leading-[1.25] tracking-[0.22em] text-[#F8F7F5]/82 md:text-[11px]"><span>{book.author}</span></p>
                        </div>
                      </div>
            ...
            <article className="rounded-[22px] border p-5 sm:p-6 md:p-8" style={{
              ...
              <figure>
                <div className="relative aspect-video overflow-hidden rounded-[18px] border" style={{
                  borderColor: COLORS.darkGray,
                  backgroundImage: "linear-gradient(135deg, #0F0F0F 0%, #212121 54%, #393939 100%)"
                }}>
                  <div className="absolute inset-0 opacity-30" style={{
                    backgroundImage: "radial-gradient(circle at 24% 30%, rgba(227, 14, 4, 0.46), transparent 24%), linear-gradient(90deg, rgba(248,247,245,0.12) 0 1px, transparent 1px 100%)",
                    backgroundSize: "auto, 42px 42px"
                  }} aria-hidden="true" />
                  <button className="absolute left-1/2 top-1/2 flex h-16 w-16 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-[#F8F7F5]/45 bg-[#0F0F0F]/45 text-[#F8F7F5] transition-colors hover:border-[#e30e04] hover:bg-[#e30e04] sm:h-20 sm:w-20" type="button" aria-label="Play The Speakers Firm Book Roundtable Series video">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-5 md:p-7" style={{
                    backgroundImage: "linear-gradient(to top, rgba(15, 15, 15, 0.96), rgba(15, 15, 15, 0))"
                  }}>
                    <h4 className={`${KONTORA_FONT_CLASS} max-w-[65ch] text-[clamp(1.375rem,6vw,1.75rem)] font-bold leading-tight tracking-[-0.04em]`} style={{
                      color: COLORS.offWhite
                    }}>
                ...
                <figcaption className="mt-4 max-w-[65ch] text-[15px] font-normal leading-[1.6] md:text-[16px] md:leading-[1.65]" style={{
                  color: COLORS.gray
                }}>
